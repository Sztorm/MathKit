...
class ComplexFTests {
    ...
    @ParameterizedTest
    @MethodSource("normalizedOrElseComplexFArgs")
    ...
    fun normalizedOrElseReturnsCorrectValue(
        complex: Wrapper<ComplexF>, defaultValue: () -> ComplexF, expected: Wrapper<ComplexF>
    ) = assertApproximation(expected.value, complex.value.normalizedOrElse(defaultValue))
    ...
    @ParameterizedTest
    @MethodSource("floatIArgs")
    fun iReturnsCorrectValue(value: Float, expected: Wrapper<ComplexF>) =
        assertEquals(expected.value, value.i)

    @ParameterizedTest
    @MethodSource("floatToComplexFArgs")
    fun toComplexFReturnsCorrectValue(value: Float, expected: Wrapper<ComplexF>) =
        assertEquals(expected.value, value.toComplexF())

    @Test
    fun sizeBitsReturnsCorrectValue() = assertEquals(ComplexF.SIZE_BITS, 64)

    @Test
    fun sizeBytesReturnsCorrectValue() = assertEquals(ComplexF.SIZE_BYTES, 8)

    @Test
    fun zeroReturnsCorrectValue() = assertEquals(ComplexF.ZERO, ComplexF(0f, 0f))

    @Test
    fun oneBytesReturnsCorrectValue() = assertEquals(ComplexF.ONE, ComplexF(1f, 0f))

    @Test
    fun imaginaryOneBytesReturnsCorrectValue() =
        assertEquals(ComplexF.IMAGINARY_ONE, ComplexF(0f, 1f))

    @ParameterizedTest
    @MethodSource("fromPolarArgs")
    fun fromPolarReturnsCorrectValue(magnitude: Float, phase: Float, expected: Wrapper<ComplexF>) =
        assertApproximation(expected.value, ComplexF.fromPolar(magnitude, phase))

    @ParameterizedTest
    @MethodSource("fromAngleArgs")
    fun fromAngleReturnsCorrectValue(angle: Wrapper<AngleF>, expected: Wrapper<ComplexF>) =
        assertApproximation(expected.value, ComplexF.fromAngle(angle.value))
    ...
    companion object {
        ...
        fun complexComponentsArgs(): List<Arguments> = listOf(
            ...
            Arguments.of(Wrapper(ComplexF(3f, -4f)), 3f, -4f),
            Arguments.of(Wrapper(ComplexF(6f, -2f)), 6f, -2f),
            Arguments.of(
        ...
        fun phaseAngleArgs(): List<Arguments> = listOf(
            Arguments.of(
                ...
                Wrapper(AngleF.fromRadians(0.9273f))
            ),
            Arguments.of(
                Wrapper(ComplexF(0f, 2f)),
                Wrapper(AngleF.fromRadians(1.5708f))
            ),
            Arguments.of(
        ...
        @JvmStatic
        fun normalizedOrElseComplexFArgs(): List<Arguments> = listOf(
            Arguments.of(
                Wrapper(ComplexF(3f, 4f)),
        ...
        @JvmStatic
        fun normalizedOrElseFunctionArgs(): List<Arguments> = normalizedOrElseComplexFArgs().map {
            val args = it.get()
            val complex = (args[1] as Wrapper<*>).value as ComplexF
            Arguments.of(args[0], { complex }, args[2])
        }
        ...
        fun complexFTimesComplexFArgs(): List<Arguments> = listOf(
            ...
            Arguments.of(
                ...
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(70f)) * 5f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(100f)) * 10f),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 2f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(-70f)) * 0.1f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(-40f)) * 0.2f),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 0.1f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(-220f)) * 4f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(170f)) * 0.4f),
            ),
        )
        ...
        fun complexFTimesVector2FArgs(): List<Arguments> = listOf(
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 2f),
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(70f)).toVector2F() * 5f
                ),
                Wrapper(
            ...
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 2f),
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(-70f)).toVector2F() * 0.1f
                ),
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(-100f)).toVector2F() * 0.2f
                ),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 0.1f),
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(-220f)).toVector2F() * 4f
                ),
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(110f)).toVector2F() * 0.4f
                ),
            ),
        )
        ...
        fun complexFDivComplexFArgs(): List<Arguments> = listOf(
            ...
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 2f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(70f)) * 5f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(-40f)) * 0.4f),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 2f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(-70f)) * 0.1f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(100f)) * 20f),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 0.1f),
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(-220f)) * 4f),
        ...
        fun complexFDivVector2FArgs(): List<Arguments> = listOf(
            Arguments.of(
                ...
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(40f)).toVector2F() * 0.4f
                ),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 2f),
                ...
                Wrapper(
                    ComplexF.fromAngle(AngleF.fromDegrees(-100f)).toVector2F() * 20f
                ),
            ),
            Arguments.of(
                Wrapper(ComplexF.fromAngle(AngleF.fromDegrees(30f)) * 0.1f),
        ...
        @JvmStatic
        fun floatIArgs(): List<Arguments> = listOf(
            Arguments.of(4f, Wrapper(ComplexF(0f, 4f))),
            Arguments.of(-4f, Wrapper(ComplexF(0f, -4f))),
            Arguments.of(-2f, Wrapper(ComplexF(0f, -2f))),
            Arguments.of(
                Float.POSITIVE_INFINITY, Wrapper(ComplexF(0f, Float.POSITIVE_INFINITY))
            ),
            Arguments.of(Float.NaN, Wrapper(ComplexF(0f, Float.NaN))),
        )

        @JvmStatic
        fun floatToComplexFArgs(): List<Arguments> = listOf(
            Arguments.of(4f, Wrapper(ComplexF(4f, 0f))),
            Arguments.of(-4f, Wrapper(ComplexF(-4f, 0f))),
            Arguments.of(-2f, Wrapper(ComplexF(-2f, 0f))),
            Arguments.of(
                Float.POSITIVE_INFINITY, Wrapper(ComplexF(Float.POSITIVE_INFINITY, 0f))
            ),
            Arguments.of(Float.NaN, Wrapper(ComplexF(Float.NaN, 0f))),
        )
        ...
        @JvmStatic
        fun log2Args(): List<Arguments> = listOf(
            Arguments.of(
                Wrapper(ComplexF(2f, 0.5f)),
                Wrapper(ComplexF(1.04373f, 0.353430f))
            ),
            Arguments.of(
                Wrapper(ComplexF(0f, -4f)),
                Wrapper(ComplexF(2f, -2.26618f))
            ),
        )
        ...
        fun acosArgs(): List<Arguments> = listOf(
            Arguments.of(
                Wrapper(ComplexF(2f, 0.5f)),
                Wrapper(ComplexF(0.277754f, -1.361801f)),
            ),
            Arguments.of(
                Wrapper(ComplexF(0f, -4f)),
